...
void solver(int n, int m, const int *p, const int *f, int &k, int *r)
{
    ...
    std::vector<int> sub_len(m);
    //first compute lengths of subsets
    sub_len[0] = p[1]; 
    for (int i = 2; i < m + 1; i++){
        sub_len[i - 1] = p[i] - p[i - 1];
    }
    //store sizes and indices of the set into priority queue
    std::priority_queue<std::pair<int, int>> collection;
    for (int i = 0; i < m; i++) {
        int len = sub_len[i]; 
        int index = i; 
        collection.push(std::make_pair(len, index));
    }

    //check if the set cover exists
    std::set<int> checker; 
    for (int i = 0; i < p[m]; i++) {
        checker.insert(f[i]);
    }
    ...
    std::vector<int> fam(p[m]);
    #pragma omp parallel for schedule(dynamic, 1)
    for (int i = 0; i < p[m]; i++) {
        fam[i] = f[i];
    }
    //greedily select sets from priority queue !! correct until this step
    ...
    int index = 0; 
    while(index < n) {
        std::pair<int, int> top_pair = collection.top();
        collection.pop();
        int lenDec = top_pair.first; 
        int sInd = top_pair.second; 
        int eInd = top_pair.second + 1;  
        for (int i = p[sInd]; i < p[eInd]; i++){
            if (universe[f[i]] != -1 && fam[i] != -1) { //Check if element is in set --- could be slow??? problem
                lenDec--; 
                fam[i] = -1;
                break;
            }
        }
        if (lenDec < top_pair.first) { 
            std::pair<int, int> p_back = std::make_pair(lenDec, top_pair.second);
            collection.push(p_back);
        } else {
            for (int i = p[sInd]; i < p[eInd]; i++) {
                if (universe[f[i]] == -1){
                    universe[f[i]] = f[i];

...
void solver(int n, int m, const char *a, const char *b, int &d)
{
    ...
    for (int iy = 0; iy < ny; iy++) {
        for (int ix = 0; ix < nx; ix++) {
            ...
            else if (a[iy - 1] == b[ix - 1]) {
                edit[ix + iy * nx] = edit[ixx + iyy * nx];
            }
            else {
                int x = edit[ixx + iy * nx]; 
                int y = edit[ix + iyy * nx]; 
                int z = edit[ixx + iyy * nx]; 
                int minimum = 1 + std::min(x, std::min(y, z)); 
                edit[ix + iy * nx] = minimum; 
            }
        }
    }

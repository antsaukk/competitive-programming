...
/*SOLUTION START O(n^2)*/
void solver(int n, const int *a_pref, const int *b_pref, int *s)
{
    std::vector<int> applicant_matchings(n);
    std::vector<int> position_matchings(n);
    std::vector<int> preference_list(n*n); 
    std::vector<int> next_application(n); 
    std::queue<int> applicants;
    int nonmatch = -1; 


    for (int i = 0; i < n; i++){
        applicant_matchings[i] = nonmatch;  //there are no employed applicants in the beginning
        position_matchings[i] = nonmatch; //there are no available positions in the beginning
        s[i] = nonmatch;
        next_application[i] = 0; //each appicant sends application to the most prefered position 
        applicants.push(i);
    }

    /*compute ranking preferences of position for applicants*/
    for (int iy = 0; iy < n; iy++){
        for (int ix = 0; ix < n; ix++){
            int xx = b_pref[ix + iy * n];
            preference_list[xx + iy * n] = ix;
        }
    }
    
    while(!applicants.empty()) {

        int applicant = applicants.front();
        int position = a_pref[next_application[applicant] + n * applicant]; 
        next_application[applicant]++;
        if (position_matchings[position] == nonmatch) {

            position_matchings[position] = applicant; 
            applicant_matchings[applicant] = position; 
            applicants.pop();

        } else if (preference_list[applicant + position * n] < preference_list[position_matchings[position] + position * n]) {

            int rejected_applicant = position_matchings[position]; 
            position_matchings[position] = applicant; 
            applicant_matchings[applicant] = position; 
            applicants.pop(); 
            applicants.push(rejected_applicant); 

        } 
    }

    for (int i = 0; i < n; i++){
        s[position_matchings[i]] = applicant_matchings[position_matchings[i]];
    } 
}
/*SOLUTION END O(n^2)*/
    ...
    while((ch = getopt_long(argc, argv, "pb", longopts, NULL)) != -1) {
        ...
        default:
            const char *ex =
                "Stable matching";
            std::cout
              << "Template for exercise '"<<ex<<"', CS-E3190 Autumn 2020\n"
              << "\n"
              << "Usage: "<<argv[0]<<" [option]\n"
              << "\n"
              << "Options:\n"

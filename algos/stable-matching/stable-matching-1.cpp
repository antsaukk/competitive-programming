...
/*SOLUTION START O(n^3)*/
void solver(int n, const int *a_pref, const int *b_pref, int *s)
{
    /*current match of applicant with position*/
    std::vector<int> amatch(n);
    std::vector<int> pmatch(n); 
    std::vector<int> next_application(n); 
    int nonmatch = -1; 


    for (int i = 0; i < n; i++){
        amatch[i] = nonmatch;  //there are no employed applicants in the beginning
        pmatch[i] = nonmatch; //there are no taken positions in the beginning
        s[i] = nonmatch;
        next_application[i] = 0; //each appicant sends application to the most prefered position 
    }

    bool available_applicants = true; 
    int applicant = 0; 
    while(available_applicants) {

        available_applicants = false; 
        int position = a_pref[(next_application[applicant]++) + n * applicant];
        if (pmatch[position] == nonmatch) {

            pmatch[position] = applicant; 
            amatch[applicant] = position; 

        } else {
            bool better_candidate = false; 
            for(int i = 0; i < n; i++) {
                if (b_pref[i + n * position] == pmatch[position]) {
                    better_candidate = false; 
                    break; 
                }
                if (b_pref[i + n * position] == applicant) {
            ...
            if (better_candidate) {
                amatch[pmatch[position]] = nonmatch; 
                pmatch[position] = applicant; 
                amatch[applicant] = position; 
            }
        } 
        ...
        for(int i = 0; i < n; i++) {
            if (amatch[i] == nonmatch) {
                applicant = i; 
                available_applicants = true; 
                break; 
            }
        }
    }

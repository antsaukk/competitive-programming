...
my tester
*/
bool verify_solution(int n, int m, int *s, int &k, const int *e, const int *c, int optatmost)
{
     if(k <= 0 || k > n) {
     	std::cout << "\nincorrect k" << std::endl; 
        return false;
     }   	
    bool good = true;
    int *b = new int[n];    
    ...
    for(int i = 0; i < k; i++) {
        if(s[i] < 0 || s[i] >= n) {
            ...
            std::cout << "\nvertex out of bounds" << std::endl; 
            good = false;
            break;                  
        } else {
            if(b[s[i]]) {
                /* Repeat vertex. */
                std::cout << "\nrepeat vertex" << std::endl; 
                good = false; 
                break;
            }
            b[s[i]] = true;
    ...
    if(cost > 2*optatmost) {
        ...
        std::cout << "\nnot within approximation bound" << std::endl; 
        good = false;
    }
    delete[] b;
    return good;
...
void solver(int n, int m, const int *e, const int *c, int &k, int *s)
{
    ...
    for (int i = 0; i < m; i++) {
    	int e1 = e[2 * i]; 
    	int e2 = e[2 * i + 1]; 
    	//pick edge which is not tight
    	if (costs[e1] != 0 && costs[e2] != 0) {
    		//increase its value until one of the vertex constraints goes tight
    		int value = std::min(costs[e1], costs[e2]);
    		costs[e1] -= value; 
    		costs[e2] -= value;
    		dual_cost += value;
    ...
    //std::cout << verify_solution(n, m, s, vertex, e, c, dual_cost) << std::endl;
    k = vertex; 
}
